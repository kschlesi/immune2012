...
% fill all test-spots in matrix with appropriate numbers
orig_tests(:,3) = orig_tests(:,3) + 2; % new key: escape = 3, chronic = 2, clear = 1;
for i=size(orig_tests,1)
    meshy = round(orig_tests(i,2)/windowsize);
    meshmatrix(orig_tests(i,2)*windowsize,orig_tests(i,1)) = orig_tests(i,3);
end
